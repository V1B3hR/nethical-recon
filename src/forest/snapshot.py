...
class ForestDiff:
    ...
    def _compute_diff(self):
        ...
        self.health_change = (
            self.new_snapshot.overall_health - self.old_snapshot.overall_health
        )
        self.threat_change = (
            self.new_snapshot.total_threats - self.old_snapshot.total_threats
        )
    ...
    def _compare_trees(self, old_tree: dict[str, Any], new_tree: dict[str, Any]) -> dict[str, Any]:
        ...
        if old_tree.get('health_score') != new_tree.get('health_score'):
            changes['health_score'] = {
                'old': old_tree.get('health_score'),
                'new': new_tree.get('health_score')
            }
        
        # Check status
        if old_tree.get('status') != new_tree.get('status'):
            changes['status'] = {
                'old': old_tree.get('status'),
                'new': new_tree.get('status')
            }
        
        # Check threat count
        if old_tree.get('threat_count') != new_tree.get('threat_count'):
            changes['threat_count'] = {
                'old': old_tree.get('threat_count'),
                'new': new_tree.get('threat_count')
            }
    ...
    def _initialize_logger(self):
        ...
        if not self.logger.handlers:
            ...
            formatter = logging.Formatter(
                "[%(asctime)s] [SnapshotManager] %(levelname)s: %(message)s"
            )
