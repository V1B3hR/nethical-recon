...
class SensorHealthMetrics:
    ...
    def record_status_change(self, new_status: SensorStatus):
        ...
        self.status_changes = [
            (ts, status) for ts, status in self.status_changes
            if ts >= cutoff
        ]
    ...
    def _calculate_health_score(self):
        ...
        recent_changes = len([
            ts for ts, _ in self.status_changes
    ...
    def _check_health_alerts(self, sensor_name: str, metrics: SensorHealthMetrics):
        ...
        if metrics.health_score <= self.alert_thresholds['health_score_critical']:
            self.logger.critical(
                f"Sensor {sensor_name} health CRITICAL: score={metrics.health_score:.1f}"
            )
        elif metrics.health_score <= self.alert_thresholds['health_score_warning']:
            self.logger.warning(
                f"Sensor {sensor_name} health DEGRADED: score={metrics.health_score:.1f}"
            )
        ...
        if metrics.check_count > 10:
            ...
            if error_rate >= self.alert_thresholds['error_rate_critical']:
                self.logger.critical(
                    f"Sensor {sensor_name} error rate CRITICAL: {error_rate*100:.1f}%"
                )
            elif error_rate >= self.alert_thresholds['error_rate_warning']:
                self.logger.warning(
                    f"Sensor {sensor_name} error rate HIGH: {error_rate*100:.1f}%"
                )
        
        # Check response time
        if metrics.avg_response_time >= self.alert_thresholds['response_time_critical']:
            self.logger.critical(
                f"Sensor {sensor_name} response time CRITICAL: {metrics.avg_response_time:.2f}s"
            )
        elif metrics.avg_response_time >= self.alert_thresholds['response_time_warning']:
            self.logger.warning(
                f"Sensor {sensor_name} response time HIGH: {metrics.avg_response_time:.2f}s"
            )
