...
class AttackChainDetector:
    ...
    def _initialize_patterns(self) -> dict[KillChainStage, list[str]]:
        ...
        return {
            KillChainStage.RECONNAISSANCE: [
                "port_scan", "network_scan", "dns_enum", "subdomain_enum",
                "shodan_query", "censys_query", "whois_lookup"
            ],
            KillChainStage.WEAPONIZATION: [
                "exploit_code", "malware_compilation", "payload_creation"
            ],
            KillChainStage.DELIVERY: [
                "phishing", "web_exploit", "malicious_attachment", "watering_hole"
            ],
            KillChainStage.EXPLOITATION: [
                "vulnerability_exploit", "code_execution", "privilege_escalation",
                "buffer_overflow", "sql_injection", "rce"
            ],
            KillChainStage.INSTALLATION: [
                "backdoor", "trojan", "rootkit", "persistence_mechanism"
            ],
            KillChainStage.COMMAND_CONTROL: [
                "c2_communication", "beacon", "callback", "reverse_shell"
            ],
            KillChainStage.ACTIONS_ON_OBJECTIVES: [
                "data_exfiltration", "lateral_movement", "credential_theft",
                "ransomware", "data_destruction"
            ]
        }
    ...
    def _classify_findings(self, findings: list[dict[str, Any]]) -> dict[KillChainStage, list[AttackChainNode]]:
        ...
        classified: dict[KillChainStage, list[AttackChainNode]] = {
