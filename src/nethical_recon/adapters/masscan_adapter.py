...
class MasscanAdapter(ToolPlugin):
    ...
    def build_command(
        ...
        cmd = [
            self.tool_path,
            target,
            "-p", options.get("ports", "1-65535"),  # Default: all ports
            "--rate", str(options.get("rate", 1000)),  # Default: 1000 packets/sec
            "-oJ", str(output_path),  # JSON output
            "--wait", "3"  # Wait for responses
        ]
