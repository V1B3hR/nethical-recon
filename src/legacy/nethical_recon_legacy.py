...
def check_tools() -> list[str]:
    ...
    for tool in required_tools:
        try:
            result = subprocess.run(["which", tool], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=5)
...
def dirb_scan(target_ip: str, wordlist_path: str | None = None) -> bool:
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert cybersecurity analyst specializing in penetration testing and vulnerability assessment. Generate accurate, actionable reports based strictly on provided scan data.",
                },
    ...
    try:
        ...
        for line in lines[-10:]:
            try:
                ...
                print(
                    f"{Fore.YELLOW}{timestamp}{Style.RESET_ALL} | {Fore.CYAN}{target}{Style.RESET_ALL} | {scan_type} | {status_color}{status}{Style.RESET_ALL}"
                )
            ...
            if choice == "1":
                ...
                try:
                    ...
                    with open(report_filename, "w", encoding="utf-8") as f:
                        ...
                        f.write("**Generated by:** Nethical v2.0\n\n")
                        f.write("---\n\n")
                        f.write(ai_report)
                    ...
                    export_html = (
                        input(Fore.YELLOW + "[?] Export report to HTML as well? (y/n): " + Style.RESET_ALL)
                        .strip()
                        .lower()
                    )
                    if export_html == "y":
