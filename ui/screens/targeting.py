...
class TargetingScreen:
    ...
    def render_target_info(self) -> Panel:
        ...
        if not self.current_target:
            return Panel(
                Text("No target selected", style=UIColors.DIM),
        ...
        if target.threat:
            text.append(
                f"Threat: {target.threat.icon} {target.threat.description.upper()}\n",
        ...
        text.append(
            f"Confidence: {conf_bar} {int(target.confidence * 100)}%\n",
            style=UIColors.TEXT
        )
    ...
    def render(self, console: Console):
        ...
        layout = Layout()
        layout.split_row(
            Layout(self.render_weapon_selector()),
            Layout(self.render_ammo_selector())
        )
