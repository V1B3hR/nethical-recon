    ...
    def scan(self, forest_data: Dict[str, Any]) -> List[BirdAlert]:
        ...
        if crow_count >= 3:
            alert = self.create_alert(
                ...
                evidence=[
                    f"Pattern: Distributed malware campaign",
                    f"Recommendation: Full forest lockdown"
                ]
            )
            alerts.append(alert)
        ...
        if squirrel_count >= 2:
            alert = self.create_alert(
                ...
                location={'scope': 'cross_tree_pattern'},
                evidence=[
                    f"Pattern: Attacker moving laterally",
                    f"Recommendation: Isolate affected trees"
                ]
            )
            alerts.append(alert)
        ...
        if len(unhealthy_trees) >= 3:
            alert = self.create_alert(
                ...
                evidence=[
                    f"Compromised trees: {len(unhealthy_trees)}",
                    f"Strategic action required"
                ]
            )
            alerts.append(alert)
