...
class HuntAdvisor:
    ...
    def _generate_action_steps(self, action: str, threat: Dict) -> List[str]:
        ...
        steps = {
            ...
            'OBSERVE': [
                ...
                '2. Log threat indicators',
                '3. Update baseline if needed'
            ],
            'PATROL': [
                '1. Maintain Sparrow routine patrols',
    ...
    def _plan_hunt_phases(self, strategy: str, threats: List[Dict]) -> List[Dict[str, Any]]:
        ...
        if strategy == 'AGGRESSIVE':
            return [
                {'phase': 1, 'name': 'CONTAIN', 'duration': '0-30 min', 'actions': ['Deploy nanobots', 'Isolate threats']},
                {'phase': 2, 'name': 'MARK', 'duration': '30-60 min', 'actions': ['Mark all threats', 'Create stains']},
                {'phase': 3, 'name': 'ELIMINATE', 'duration': '1-2 hours', 'actions': ['Neutralize threats', 'Verify elimination']}
            ]
        elif strategy == 'SYSTEMATIC':
            return [
                {'phase': 1, 'name': 'PRIORITIZE', 'duration': '0-15 min', 'actions': ['Rank threats', 'Plan sequence']},
                {'phase': 2, 'name': 'EXECUTE', 'duration': '1-4 hours', 'actions': ['Process threats one by one']},
                {'phase': 3, 'name': 'VERIFY', 'duration': '30-60 min', 'actions': ['Confirm elimination', 'Update database']}
            ]
        else:
            return [
    ...
    def recommend_bird_deployment(self, situation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'coordination': 'Birds should report to Eagle if deployed' if any(d['bird'] == 'EAGLE' for d in deployments) else 'Independent operation',
