...
def example_4_scout_mode():
    ...
    enumerator = EnumeratorNanobot(
        config={'max_concurrent': 3}
    )
    
    forest_patrol = ForestPatrolNanobot(
        config={'patrol_interval': 60}
    )
    
    threat_hunter = ThreatHunterNanobot(
        config={'hunt_types': ['crow', 'magpie', 'squirrel']}
    )
