...
class PostgreSQLStore(BaseStore):
    ...
    def _row_to_dict(self, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'forest_location': row['forest_location'] if isinstance(row['forest_location'], dict) else (json.loads(row['forest_location']) if row['forest_location'] else {}),
            'stain': {
                'threat_score': float(row['threat_score']),
                'confidence': float(row['confidence']),
                'evidence': row['evidence'] if isinstance(row['evidence'], list) else (json.loads(row['evidence']) if row['evidence'] else []),
                'linked_tags': row['linked_tags'] if isinstance(row['linked_tags'], list) else (json.loads(row['linked_tags']) if row['linked_tags'] else [])
            },
