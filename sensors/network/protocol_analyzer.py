...
class ProtocolAnalyzer(BaseSensor):
    ...
    def _analyze_http(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ['timeout', '5', 'tcpdump', '-i', self.interface, '-n', 
                 'tcp', 'port', '80', 'or', 'port', '8080', '-c', '20'],
                capture_output=True,
                text=True,
                timeout=10
            )
    ...
    def _analyze_dns(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for packet in packets:
                ...
                if len(packet) > 200:
                    anomalies.append({
                        'type': 'suspicious_long_query',
                        'data': packet[:100]  # Truncate for logging
                    })
    ...
    def _handle_protocol_anomaly(self, protocol: str, finding: Dict[str, Any]):
        ...
        self.raise_alert(
            severity,
            f'Protocol anomaly in {protocol}: {anomaly_type}',
            {
                'protocol': protocol,
                'finding': finding
            }
