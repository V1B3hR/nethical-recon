...
class APIKeyRotator:
    ...
    def _initialize_logger(self):
        ...
        if not self.logger.handlers:
            ...
            formatter = logging.Formatter(
                f"[%(asctime)s] [KeyRotator:{self.api_name}] %(levelname)s: %(message)s"
            )
    ...
    def add_key(self, key_id: str, key_value: str, daily_limit: int | None = None) -> bool:
        ...
        self.logger.info(
            f"Added key {key_id} (daily_limit={daily_limit or 'unlimited'})"
        )
        return True
    ...
    def record_error(self, key_value: str):
        ...
        for key in self.keys.values():
            if key.key_value == key_value:
                key.record_error()
                self.logger.warning(
                    f"Error recorded for key {key.key_id} "
                    f"(errors: {key.error_count}, active: {key.is_active})"
                )
                break
    ...
    def record_rate_limit(self, key_value: str, cooldown_seconds: float = 60.0):
        ...
        for key in self.keys.values():
            if key.key_value == key_value:
                ...
                self.logger.warning(
                    f"Rate limit hit for key {key.key_id}, "
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        total_capacity_today = sum(
            k.get_remaining_today() or 0
